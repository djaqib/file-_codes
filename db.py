...
from config import DATABASE_URL, SHARE_CODE_LENGTH, SHARE_CODE_ALPHABET
...
def add_item(session_id: int, vault_chat_id: int, vault_message_id: int, file_type: str, caption: str | None):
    with get_cursor(commit=True) as cur:
        cur.execute(
            """INSERT INTO items (session_id, vault_chat_id, vault_message_id, file_type, caption)
               VALUES (%s, %s, %s, %s, %s) RETURNING *""",
            (session_id, vault_chat_id, vault_message_id, file_type, caption),
        )
        return cur.fetchone()
